...
ann_file_train = DATASET_PATH + '50_few_shot_depth_train_ann.txt'
ann_file_val = DATASET_PATH + '50_few_shot_depth_val_ann.txt'
ann_file_test = DATASET_PATH + '50_few_shot_depth_val_ann.txt'

#ann_file_train = DATASET_PATH + 'depth_small_train_ann.txt'
#ann_file_val = DATASET_PATH + 'depth_small_valid_ann.txt'
...
data = dict(
    videos_per_gpu=8,#8,
    workers_per_gpu=2,#2, # Changed from 4
    val_dataloader=dict(
        videos_per_gpu=2,
        workers_per_gpu=1
    ),
...
# optimizer
# Modify training schedule? Same as before... (changed 1e-4 --> 1e-3)
...
# Modify workflow to calculate validation loss
...
# runtime settings
checkpoint_config = dict(interval=1) # Don't save...
work_dir = WORK_DIR_PATH + '50_few_shot_nturgbd_swin_tiny_depth.py'
