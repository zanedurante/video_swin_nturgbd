...
@PIPELINES.register_module()
class CopyDims():
    # Num times to copy last dimension (input channels) across
    ...
    def __call__(self, results):
        ...
        imgs = np.empty((n, h, w, c * self.num_times), dtype=np.float32)
        for i, img in enumerate(results['imgs']):
            imgs[i] = np.repeat(img, self.num_times, axis=-1)
...
class Normalize:
    ...
    def __call__(self, results):
        ...
        if modality == 'Depth':
            n = len(results['imgs'])
            h, w = results['imgs'][0].shape # Expect no channels for grayscale images
            c = 1 # For grayscale images, c == 1
            imgs = np.empty((n, h, w, c), dtype=np.float32)
            for i, img in enumerate(results['imgs']):
                imgs[i] = np.expand_dims(img, axis=-1) # Add grayscale channel
