...
class RawframeTripletDepthDatasetPNG(BaseDataset):
    ...
    def load_annotations(self):
        ...
        self.label_to_idxs = {label: np.where(np.asarray(self.labels) == label)[0]
    ...
    # Gets positive and negative examples for contrastive learning
    def get_pos_neg(self, idx):
        label = self.labels[idx]
        pos_idx = idx
        while pos_idx == idx:
            pos_idx = np.random.choice(self.label_to_idxs[label])
        neg_label = np.random.choice(list(self.labels_set - set([label])))
        neg_idx = np.random.choice(self.label_to_idxs[neg_label])
        return pos_idx, neg_idx
    
    # Returns three indexes (anchor (idx), pos_example_idx, neg_example_idx (randomly sampled))
    def __getitem__(self, idx):
        """Get the sample for either training or testing given index."""
        if self.test_mode:
            return self.prepare_test_frames(idx)
        
        pos_idx, neg_idx = self.get_pos_neg(idx)
        results = self.prepare_train_frames(idx)
        pos_results = self.prepare_train_frames(pos_idx)
        neg_results = self.prepare_train_frames(neg_idx)
        results['pos_imgs'] = pos_results['imgs']
        results['neg_imgs'] = neg_results['imgs']
